...
int Cashier(int cant, int largo, char estadoCajeras[], char marketName[], int llegadaClientes, int tiempoAtencion[], int cantClientesLlegados, int cantClientesAtendidos){
	...
	printf("|Nombre del supermercado: %s|\t|Recibiendo 1 cliente cada %d milisegundos|\t\t|Han llegado %d clientes |\t|%d clientes han sido atendidos\t|", marketName, llegadaClientes, cantClientesLlegados, cantClientesAtendidos);
...
system("cls");//Se limpia la pantalla antes de comenzar el programa
	...
	while(fin == 'N'){//Se hace un ciclo que representara el tiempo en que esta funcionando el supermercado
		tiempo = tiempo + 50;//Se aumenta en 50 el tiempo cada que entre en el ciclo (Se tuvo que hacer así porque de otra forma no era congruente)
	...
	MoverCursor(30,44);//Se mueve el cursor para poder terminar en programa sin que se empalmen los textos
